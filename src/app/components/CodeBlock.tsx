...
export function MDXCodeBlock({
    ...
    const filename = meta.match(/filename="([^"]+)"/)?.[1] ||
        meta.match(/file=([^\s]+)/)?.[1] ||
        '';
    const highlightLines = meta.match(/lines="([^"]+)"/)?.[1]?.split(',').map(Number) || [];
    ...
    return (
        <Suspense>
            <div className={code['mdx-code-block'] + ' ' + code['mdx-code-block-loaded']}>
                ...
                <div className={code['mdx-code-content'] + ((filename || lang !== 'txt') ? ' ' + code['mdx-code-content-with-header'] : '')}>
                    <div
                        className={code['mdx-highlighted-code']}
                    ...
                    {!(filename || lang !== 'txt') && (
                        <button
                            onClick={copyToClipboard}
                            className={code['mdx-floating-copy-button'] + (copied ? ' ' + code['mdx-floating-copy-button-copied'] : '')}
                            aria-label={copied ? "Copied!" : "Copy code"}
                        >
